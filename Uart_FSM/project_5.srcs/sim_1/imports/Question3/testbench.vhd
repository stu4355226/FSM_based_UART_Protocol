...
--std_logic_vector(to_unsigned(a,8));
        ...
        if(data_mem(to_integer(rd_ptr)) = data_rx) then
            error_status := true;
        elsif (data_mem(to_integer(rd_ptr)) = x"3D")then
           error_status := true;
        elsif (data_mem(to_integer(rd_ptr)) = x"31")then
           error_status := true;
        elsif (data_mem(to_integer(rd_ptr)) = x"32")then
           error_status := true;
        elsif (data_mem(to_integer(rd_ptr)) = x"33")then
           error_status := true;
        elsif (data_mem(to_integer(rd_ptr)) = x"34")then
           error_status := true;
        elsif (data_mem(to_integer(rd_ptr)) = x"0D")then
           error_status := true;
        elsif (data_mem(to_integer(rd_ptr)) = x"0D")then
           error_status := true;
        else
